...
import { createGardenBed } from '../api/gardens'
...
export function CreateGardenBedPage() {
  ...
  const mutation = useMutation({
    mutationFn: (body: Parameters<typeof createGardenBed>[1]) => createGardenBed(id!, body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['garden', id] })
      navigate(`/gardens/${id}`)
    },
  })
          ...
          <form onSubmit={handleSubmit} noValidate>
            <Stack sx={{ gap: 3 }}>
              ...
              <Stack direction="row" sx={{ gap: 2, justifyContent: 'flex-end', mt: 1 }}>
                ...
                >
                  {mutation.isPending ? 'Creating…' : 'Add bed'}
                </Button>
              </Stack>
            </Stack>
          </form>
